...
6. While still in the source checkout, run bundled `scripts/copy_worktreeinclude.py` with the new worktree path: `python <skill-root>/scripts/copy_worktreeinclude.py --source-root "$(git rev-parse --show-toplevel)" .worktrees/<branch-name>` (or `.worktrees/<slug>`). Preserve any warning or error output.
7. Switch into it: `cd .worktrees/<branch-name>` (or `.worktrees/<slug>`).
